<template>
  <div class="media dialog-item" :data-message="dialog.lastMessage.id">
    <div class="media-body">
      <router-link class="dialog-author" :to="!$store.state.edit ? `/profile/id${dialog.secondUser.id}` : ''" :data-author="dialog.secondUser.id">
        <AvatarMin style="
                height: 55px !important;
                width: 55px !important;
                vertical-align: unset;
              " :userId="dialog.secondUser.id" :lastVisit="dialog.secondUser.ulastVisit" :photo="dialog.secondUser.photo" />
      </router-link>
      <router-link style="flex: 1" :to="!$store.state.edit ? `/messages/id${dialog.secondUser.id}` : ''">
        <div style="display: inline-block; margin-left: 5px; width: 100%">
          <div class="media-heading">
            <router-link class="dialog-author" :to="
                    !$store.state.edit ? `/profile/id${dialog.secondUser.id}` : ''
                  " :data-author="dialog.secondUser.id">
              <div class="dialogist" v-if="dialog.secondUser.id != myId">
                {{ `${dialog.secondUser.firstName} ${dialog.secondUser.lastName}` }}
              </div>
              <div class="dialogist" v-else>Я</div>
            </router-link>
          </div>
          <div class="container-message" :class="
                  dialog.lastMessage.senderId === myId ? 'sended' : 'received'
                ">
            <div class="last-message text-justify" :class="{'unread' : dialog.lastMessage.unread}">
              <span>{{
                    dialog.lastMessage.senderId === myId
                      ? 'Я'
                      : dialog.secondUser.sex
                      ? 'Она'
                      : 'Он'
                  }}</span
                  >:
            ...
            <span v-if="
                  ...
                  " class="unread-counter">{{ dialog.unreadCount }}</span
                >
              </div>
              <span class="time">{{ timeName(dialog.lastMessage.utime) }}</span>
          </div>
      </router-link>
      </div>
    </div>
