<template>
  <div>
    <div class="row col-12" id="posts-head">
      <h4 v-if="user.posts">Посты ({{ user.postsCount }})</h4>
      <div id="post_add_wrapper">
        <AddBtn v-if="user.id === myId" @click="slide" data-bs-toggle="tooltip" data-bs-placement="top" title="Добавить пост" id="btn_post-add" ref="post_add" />
      </div>
    </div>
    <collapse-transition>
      <div v-if="user.id === myId && isSlided" class="post-add-form" v-click-outside="collapse">
