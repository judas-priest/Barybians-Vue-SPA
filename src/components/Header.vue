<template>
  <div v-cloak v-if="$store.state.myId">
    <nav class="navbar navbar-expand-sm navbar-expand-lg fixed-top snowflakes" id="nav-top" v-if="!$store.state.mobile">
      <div class="container" :class="{ page__loading: loading }">
        <router-link class="navbar-brand logo" :to="!$store.state.edit ? `/profile/id${myId}` : ''">Барыбинцы</router-link>
  
        <ul class="navbar-nav">
          <li class="nav-item">
            <router-link class="nav-link" :class="{ 'nav-link-active': $route.name == 'Dialogs' }" :to="!$store.state.edit ? '/dialogs' : ''">Сообщения
              <sup class="nav-msg-not" v-show="messagesCount > 0">{{
                    messagesCount
                  }}</sup></router-link>
          </li>
          <li class="nav-item" id="nav-users">
            <span class="nav-link pointer" :class="{ 'nav-link-active': isSlided }" @click="isSlided = !isSlided && !$store.state.edit" id="btn_all-users" ref="user">Обитатели</span
                >
              </li>
      ...
      <Friends :slided="isSlided" @outside="outside" v-if="myId" />
    </nav>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-bottom snowflakes" id="nav-bottom" :class="{ page__loading: loading }" v-else>
      <div class="container" id="nav-container">
        <span class="nav-btn" :class="{ 'nav-btn-prsd': $route.name === 'Profile' }">
              <router-link :to="!$store.state.edit ? `/profile/id${myId}` : ''">
                <img class="nav-btn-img" src="/img/nav/budka.svg" alt="profile" />
              </router-link>
            </span>
        <span class="nav-btn" :class="{ 'nav-btn-prsd': $route.name === 'Dialogs' }">
        ...
        <span id="nav-friends" class="nav-btn" :class="{ 'nav-btn-prsd': isSlided }">
              <img
        ...
        <span id="nav-feed" class="nav-btn" :class="{ 'nav-btn-prsd': $route.name === 'Feed' }">
              <router-link :to="!$store.state.edit ? '/feed' : ''">
                <img class="nav-btn-img" src="/img/nav/feed.svg" alt="feed" />
              </router-link>
            </span>
        <span id="nav-settings" class="nav-btn" :class="{ 'nav-btn-prsd': $route.name === 'Settings' }">
