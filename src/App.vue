<template>
  <div>
    <Header v-if="currentRouteName !== 'Index' && currentRouteName !== undefined" />
    <div id="preloader" class="visible transition" :style="bdk"></div>
    <router-view v-slot="{ Component }">
      <transition name="fade" mode="out-in">
        <component :is="Component" :class="{ position__fixed: $store.state.modal.toggle }" />
      </transition>
    </router-view>
    ...
    mounted() {
      this.emitter.on('loaded', () => {
        if (
          ...
          this.$store.state.loaded.content && !this.loaded
        ) {
          this.preloaderStop()
