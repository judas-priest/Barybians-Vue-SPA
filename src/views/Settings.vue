    ...
    <div class="row">
      <div class="col-12">
        ...
        <ul v-on:submit.prevent="submitForm" id="settings-wrapper">
          <li type="none" v-for="item in items" :key="item.name" class="setting-item">
            <h5 class="settings-title">{{ item.label }}</h5>
            <Item :item="item" v-model:checked="item.checked" @change="changed = true" />
          </li>
          <div id="setting-buttons-wrapper">
            <button class="btn btn-outline-danger col-12 col-lg-2" @click="resetValues">
                Отменить
              </button>
            <button class="btn btn-success col-12 col-lg-2" @click="setValues" :disabled="changed === false">
                Применить
              </button>
          </div>
        </ul>
